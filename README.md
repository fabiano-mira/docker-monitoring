...
3. If no UI option exists, SSH into the router and locate its flow-exporter config (path varies by UDR/UDM firmware, commonly under the `ubios-udapi-server` or `vnstatd` flow-export settings):
   ...
   ssh root@10.9.1.1
   ```
4. Set the exporter's destination to `<mac-lan-ip>:2057`, protocol **NetFlow v9 — not IPFIX**. IPFIX flows reach ntopng with zero byte/packet counters, leaving Top Talkers and all traffic-volume views empty (see "IPFIX vs NetFlow v9" under Known limitations).
...
- **Export NetFlow v9 from the router, not IPFIX — netflow2ng sends 0-byte flows to ntopng for IPFIX.** netflow2ng's ntopng field mapping (`formatter/mapping.yaml`) only remaps `IN_BYTES`/`IN_PKTS`/`OUT_BYTES`/`OUT_PKTS` for NetFlow v9; for IPFIX (v10) those custom fields stay at zero, so ntopng receives every flow with `0/0 pkts, 0/0 bytes`. Symptoms (observed on this deployment): ntopng shows hosts, active flows, ports, and nDPI protocols, but Top Talkers is empty, all host/flow byte counters read 0, and interface `bytes` stays 0 — while goflow2's `flows.log` shows correct byte counts from the very same packets (goflow2 fully supports IPFIX; only the netflow2ng→ntopng leg is affected). Diagnosis shortcut: `docker logs ntopng` showing flows tagged `[0/0 pkts][0/0 bytes]` plus netflow2ng metrics reporting `version="10"` flowsets confirms this exact issue. Fix: switch the router's flow export from IPFIX to **NetFlow v9** (same destination IP and port `2057`). An upstream fix adding an `ipfix:` mapping block exists ([netflow2ng PR #162](https://github.com/synfinatic/netflow2ng/pull/162)) but is unmerged and absent from the published Docker image, so exporting v9 is the required workaround until it ships.
