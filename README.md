...
| `netflow2ng` | `netflow2ng:ipfix-patched` (built from `netflow2ng-build/`) | 2056 (udp), 8091 | Receives NetFlow/IPFIX, republishes flows to ntopng over ZMQ. Locally-built with the IPFIX byte-counter patch — see Known limitations |
...
3. If no UI option exists, SSH into the router and locate its flow-exporter config (path varies by UDR/UDM firmware, commonly under the `ubios-udapi-server` or `vnstatd` flow-export settings):
   ...
   ssh root@10.9.1.1
   ```
4. Set the exporter's destination to `<mac-lan-ip>:2057`, protocol NetFlow v9 or IPFIX. (IPFIX only works because this stack builds a patched netflow2ng image — the stock image sends 0-byte flows to ntopng for IPFIX; see Known limitations.)
...
| `netflow2ng-build/` | Self-contained Docker build context for the IPFIX-patched netflow2ng image (`docker compose build netflow2ng`): clones upstream, overlays `mapping.yaml` with the `ipfix:` block from unmerged PR #162 |
...
- **The stock `synfinatic/netflow2ng` image sends 0-byte flows to ntopng for IPFIX — this stack builds a patched image instead.** Upstream netflow2ng's ntopng field mapping (`formatter/mapping.yaml`) only remaps `IN_BYTES`/`IN_PKTS`/`OUT_BYTES`/`OUT_PKTS` for NetFlow v9; for IPFIX (v10) those custom fields stay at zero, so ntopng receives every flow with `0/0 pkts, 0/0 bytes`. Symptoms (observed and fixed on this deployment): ntopng shows hosts, active flows, ports, and nDPI protocols, but Top Talkers is empty, all host/flow byte counters read 0, and interface `bytes` stays 0 — while goflow2's `flows.log` shows correct byte counts from the very same packets (goflow2 fully supports IPFIX; only the netflow2ng→ntopng leg is affected). Diagnosis shortcut: `docker logs ntopng` showing flows tagged `[0/0 pkts][0/0 bytes]` plus netflow2ng metrics reporting `version="10"` flowsets. Fix (in place): `docker-compose.yml` builds `netflow2ng:ipfix-patched` from `netflow2ng-build/`, which overlays the `ipfix:` mapping block from unmerged upstream [PR #162](https://github.com/synfinatic/netflow2ng/pull/162); verified end-to-end — after the router's next template refresh, ntopng byte counters and Top Talkers populated. Once that PR ships in the published image, the `build:` override can be dropped. Alternative workaround without the patched build: export NetFlow v9 from the router instead of IPFIX.
- **After recreating the `netflow2ng` container, ntopng's live view stays empty until the router re-sends its IPFIX/NetFlow templates** (the container's template cache is in-memory). Options data may resume quickly, but data flowsets log `template_not_found` until the *data* template refresh arrives — which can take 10–20 minutes on UniFi. Toggling the router's flow-export setting off/on forces an immediate template resend. Historical data (`flows.log`, Loki, both InfluxDBs) is unaffected; native goflow2 keeps its own template cache and continues decoding throughout.
